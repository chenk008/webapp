...
public class MyJedisClusterInfoCache {
	...
	public MyJedisClusterInfoCache(final GenericObjectPoolConfig poolConfig, int timeout) {
		this(poolConfig, timeout, timeout, null);
	}

	public MyJedisClusterInfoCache(final GenericObjectPoolConfig poolConfig, final int connectionTimeout,
	...
	public void renewClusterSlots(Jedis jedis) {
		// If rediscovering is already in process - no need to start one more
		// same rediscovering, just return
		if (!rediscovering) {
			try {
				w.lock();
				rediscovering = true;
	...
	public JedisPool setupNodeIfNotExist(HostAndPort node) {
		...
		try {
			...
			JedisPool nodePool = new JedisPool(poolConfig, node.getHost(), node.getPort(), connectionTimeout, soTimeout,
					password, 0, null, false, null, null, null);
	 ...
	 * @param targetNode
	 */
	public void assignSlotsToSlaveNode(List<Integer> targetSlots, HostAndPort targetNode) {
		w.lock();
		try {
			JedisPool targetPool = setupNodeIfNotExist(targetNode);
			for (Integer slot : targetSlots) {
				List<JedisPool> list = slaveSlots.get(slot);
				if (list == null) {
					list = new ArrayList<JedisPool>();
					slaveSlots.put(slot, list);
				}
				if (!list.contains(targetPool)) {
					list.add(targetPool);
				}
			}
		} finally {
	...
	public JedisPool getSlaveSlotPool(int slot) {
		r.lock();
		try {
			List<JedisPool> list = slaveSlots.get(slot);
			if (list != null && list.size() != 0) {
				return list.get(slot % list.size());
			}
			return null;
		} finally {
			r.unlock();
